...
<div class="max-w-5xl mx-auto py-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div>
            ...
            @elseif(optional($moto->photos)->count())
                <img src="{{ asset('storage/' . $moto->photos->first()->path) }}" 
                     alt="{{ $moto->title }}" 
                     class="w-full h-64 object-cover rounded-lg mb-4">
            @else
        ...
        <div>
            ...
            <p><span class="font-bold">Estado:</span> {{ ucfirst($moto->condition) }}</p>

            <p class="text-3xl text-red-600 font-bold mt-4">
                €{{ number_format($moto->price_cents / 100, 0, ',', '.') }}
            </p>
