...
<form action="{{ route('listings.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
    ...
    <div>
        <label for="condition" class="block">Condición</label>
        <select name="condition" id="condition" class="border p-2 w-full">
    ...
    <div>
        <label for="price_cents" class="block">Precio (€)</label>
        <input type="number" name="price_cents" id="price_cents" class="border p-2 w-full" required>
    </div>
