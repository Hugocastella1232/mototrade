...
@if(count($carrito) > 0)
        ...
        @foreach($carrito as $id => $item)
            <li class="bg-white shadow p-4 rounded flex justify-between items-center">
                <div>
                    ...
                    <span class="text-gray-600"> - €{{ number_format($item['price_cents'] / 100, 2) }}</span>
    ...
    <div class="mt-6 text-right">
        <p class="text-lg font-bold">
            Total: €{{ number_format(collect($carrito)->sum(fn($item) => $item['price_cents'] * $item['quantity']) / 100, 2) }}
        </p>
    </div>
